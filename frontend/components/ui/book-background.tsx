...
export default function AnimatedBookBackground() {
  ...
  const generateBooks = () => {
    const booksArray = [];
    const colors = [
      "bg-amber-700", "bg-emerald-800", "bg-indigo-700", "bg-rose-700", 
      "bg-purple-800", "bg-blue-800", "bg-teal-700", "bg-red-900"
    ];
    ...
    for (let i = 0; i < 24; i++) {
      ...
      booksArray.push({ id: i, x, y, width, height, color, delay, rotate });
    }
  ...
  if (!mounted) {
    return <div className="fixed inset-0 overflow-hidden -z-10 bg-gradient-to-br from-slate-100 to-slate-200 opacity-70" />;
  }
