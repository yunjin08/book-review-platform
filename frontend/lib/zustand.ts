...
type ExtendStore<T, TExtension extends Record<string, unknown>> = (
...
type CreateGenericStoreReturn<
    T,
    TExtension extends Record<string, unknown>,
> = UseBoundStore<StoreApi<GenericState<T> & GenericActions<T> & TExtension>>

// Update function signature to accept TExtension generic
export const createGenericStore = <
    T extends { id?: string | number },
    TExtension extends Record<string, unknown> = Record<string, never>, // Default to empty object if no extension
>(
    endpoint: string,
    options?: {
        actions?: ActionType[]
    ...
    const defaultExtend = (() => ({}) as TExtension) as ExtendStore<
        T,
        TExtension
    >

    // Default actions include all available actions if none specified
    const availableActions: ActionType[] = options?.actions || [
        'fetchAll',
        'fetchOne',
        'create',
        'update',
    ...
    return create<FullStoreState>((set, get) => {
        ...
        if (availableActions.includes('fetchAll')) {
            store.fetchAll = async (params: Record<string, unknown> = {}) => {
                ...
                try {
                    ...
                    const data: ApiResponse = await apiClient.get<ApiResponse>(
                        `${endpoint}/`,
                        params
                    )
                    set({
        ...
        if (availableActions.includes('create')) {
            store.create = async (payload: Partial<T>) => {
                ...
                try {
                    createdData = await apiClient.post<T>(
                        `${endpoint}/`,
                        payload
                    )
                    ...
                    if (
                        'fetchAll' in state &&
                        typeof state.fetchAll === 'function'
                    ) {
        ...
        if (availableActions.includes('update')) {
            store.update = async (id: string | number, payload: Partial<T>) => {
                ...
                try {
                    updatedData = await apiClient.put<T>(
                        `${endpoint}/${id}/`,
                        payload
                    )
                    set({ loading: false } as Partial<FullStoreState>)
        ...
        return {
            ...
            ...(options?.extendStore
                ? options.extendStore(set, get)
                : defaultExtend(set, get)),
        }
    })
}
