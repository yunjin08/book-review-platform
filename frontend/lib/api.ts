...
interface ApiClientConfig extends Omit<AxiosRequestConfig, 'baseURL'> {
  baseURL: string;
  requestInterceptors?: (config: InternalAxiosRequestConfig) => Promise<InternalAxiosRequestConfig> | InternalAxiosRequestConfig;
...
export const initApiClient = (config: ApiClientConfig): void => {
  const { baseURL, requestInterceptors, requestInterceptorErrors, responseInterceptors, responseInterceptorErrors, ...restConfig } = config;
